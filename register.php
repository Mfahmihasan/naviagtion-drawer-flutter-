...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["new-username"]) && isset($_POST["new-password"]) && isset($_POST["confirm-password"]) && isset($_POST["name"]) && isset($_POST["token"])) {
        ...
        if ($password != $confirmPassword) {
            ...
            if (mysqli_num_rows($result) > 0) { 
                ...
                $query = "INSERT INTO tbl_user(username, password, name, 'token') VALUES (?, ?, ?,?)";
                $stmt = mysqli_prepare($koneksi, $query);

                if ($stmt) {
                    mysqli_stmt_bind_param($stmt, "ssss", $username, $password, $name, $token);
...
<body>
        ...
        <div class="container">
            ...
            <div class="register-container">
                <form id="register-form" action="register.php" method="post">
                    ...
                    <input for="session-token">Session Token:</label>
    ...
    <!-- Footer -->
    <footer id="footer">
        <p class="copyright">&copy; NAMA: MUHAMAD FAHMI HASAN</p>
        <p>NPM: 21552011365</p>
